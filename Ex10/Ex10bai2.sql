...
-- 2) tạo view tính tổng bài viết cho từng user
create view view_user_post as
select user_id, count(user_id) as total_user_post
from posts
group by user_id;

-- 3) hiển thị view vừa tạo để kiểm chứng
select user_id, total_user_post 
from view_user_post;

-- 4) join view với bảng users để lấy tên và số lượng bài
