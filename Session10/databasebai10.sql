...
VALUES(
	'hoang', 'Nguyen Duc Huy Hoang', 'Nam', 'Hoang@gmail.com', '123', '1990-01-01', 'Hà Nội');
    -- Xóa dữ liệu
delete from users where username ='hoang';    

-- Bài 2: Tạo một view tên view_user_post
Create view view_user_post
as 
select user_id, count(*) as total_user_post  from posts
group by user_id;
...
from users
inner join view_user_post on users.user_id=view_user_post.user_id;

-- Bài 3: Viết câu truy vấn Select tìm tất cả những User ở Hà Nội
EXPLAIN ANALYZE
select * from users 
where hometown = 'Hà Nội';
-- -> Filter: (users.hometown = 'Hà Nội')  (cost=2.75 rows=2.5) (actual time=0.0378..0.0744 rows=8 loops=1)
--      -> Table scan on users  (cost=2.75 rows=25) (actual time=0.0338..0.0648 rows=25 loops=1)
...
-- '-> Index lookup on users using idx_hometown (hometown=\'Hà Nội\')  (cost=1.43 rows=8) (actual time=0.0274..0.0344 rows=8 loops=1)\n'

DROP INDEX idx_hometown ON users;

-- Bài 4:

-- Tạo một truy vấn để tìm tất cả các bài viết (posts) trong năm 2026
EXPLAIN ANALYZE
