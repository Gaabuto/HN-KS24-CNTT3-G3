...
-- ========================================================================================
/*
	1.Tạo một view tên view_user_post hiển thị danh sách các User với 
các cột: user_id(mã người dùng) và total_user_post (Tổng số bài viết mà từng người dùng đã đăng).
*/
...
-- 2. Tiến hành hiển thị lại view_user_post để kiểm chứng
SELECT * FROM social_network_pro.view_user_post;
/*
	3. Kết hợp view view_user_post với bảng users để hiển thị 
các cột: full_name(họ tên) và  total_user_post (Tổng số bài viết mà từng người dùng đã đăng).
*/
select u.full_name, v.total_user_post
from users as u join view_user_post as v on u.user_id = v.user_id;
