...
-- bai 1
delimiter //
-- tao procedure lay id tra ve thong tin
create procedure stored_procedure( id_in int)
begin
...
delimiter //
create procedure CalculatePostLikes (id_in int )
begin
...
delimiter //
create procedure CalculateBonusPoints (id_in int, inout bonus_point int)
begin
declare post_count int;
select count(post_id) into post_count
from posts
where user_id = id_in;

if post_count >= 20 then set bonus_point = bonus_point + 100;
elseif post_count >= 10 then set bonus_point = bonus_point + 50;
end if;
end //
delimiter ;

set @bonustest = 10;
call CalculateBonusPoints(1, @bonustest);
select @bonustest;
...
delimiter //
create procedure CreatePostWithValidation (id_in int, context_in text, out result_message text)
begin
 if char_length(context_in) < 5 then set result_message = 'Nội dung quá ngắn';
 else 
 insert into post(post_id, context)
 values
 (id_in, context_in);
...
call CreatePostWithValidation(1, 'testtesttesttesttesttest', @test);
select @test;
...
select count(comment_id) into count_comments
from comments
where user_id = id_in;

select count(user_id) into count_likes
    from posts p
