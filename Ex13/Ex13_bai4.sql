drop database if exists ss13_ex4;
...
create table post_history(
    ...
    primary key(history_id, post_id),
    foreign key (post_id) references posts(post_id)
    on update cascade
    on delete cascade
...
delimiter $$
drop trigger if exists posts_before_update_history $$
create trigger posts_before_update_history
...
drop trigger if exists posts_after_delete_log $$
create trigger posts_after_delete_log
after delete on posts
for each row
begin
  insert into post_history (post_id, old_content, new_content, changed_at, changed_by_user_id)
...
update posts set content = 'Bob first post (updated content)' where post_id = 3;
