DROP DATABASE IF EXISTS session13_ex6;
CREATE DATABASE session13_ex6;
USE session13_ex6;
...
begin
    ...
    if NEW.username NOT REGEXP '^[a-zA-Z0-9_]+$' THEN
        signal sqlstate '45000' set message_text = 'Username chỉ được chứa chữ, số và dấu gạch dưới!';
    end if;
end //

CREATE PROCEDURE add_user(in p_username VARCHAR(50), in p_email VARCHAR(100), in p_date DATE)
BEGIN
    INSERT INTO users (username, email, created_at) VALUES (p_username, p_email, p_date);
END //
...
CREATE TRIGGER trigger_after_insert_friendship
...
CREATE PROCEDURE follow_user(IN p_follower_id INT, IN p_followee_id INT)
BEGIN
    IF p_follower_id = p_followee_id THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Không thể tự follow chính mình!';
    END IF;

    INSERT IGNORE INTO friendships (follower_id, followee_id) VALUES (p_follower_id, p_followee_id);
END //
...
CALL add_user('bob!!', 'bob@gmail', '2025-01-01');

-- Bài 6
CREATE VIEW user_profile AS
SELECT u.user_id, u.username, u.email, u.post_count, u.follower_count
FROM users u;
INSERT INTO users (username, email, created_at) 
VALUES ('alicede', 'alicede@gmail.com', '2026-01-01'), 
('bobede', 'bobede@gmail.com', '2026-01-01');
